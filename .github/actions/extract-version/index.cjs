...
const parseVersion = (version) => {
	const match = version.match(
		/^(refs\/tags\/)?v?(?<major>[0-9]+)\.(?<minor>[0-9]+)\.(?<patch>[0-9]+)(-(?<prerelease>[a-z]+)\.(?<prereleaseVersion>[0-9]+))?$/,
	)
	if (!match) {
		return null
	}
...
;(() => {
	...
	exec('git tag -l', (err, stdout) => {
		...
		const latestPrereleaseVersion = version.prerelease && samePatchAndStability.length > 0
			? Math.max(...samePatchAndStability.map(it => it.prereleaseVersion))
			: -1

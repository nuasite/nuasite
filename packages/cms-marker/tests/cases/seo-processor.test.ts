...
describe('processSeoFromHtml', () => {
	describe('title extraction', () => {
		test('extracts title content', () => {
			...
			const result = processSeoFromHtml(html)

			expect(result.seo.title).toBeDefined()
			expect(result.seo.title?.content).toBe('My Page Title')
		})

		test('marks title with CMS ID when markTitle is true', () => {
			const html = '<html><head><title>My Page Title</title></head><body></body></html>'
			let counter = 0
			const result = processSeoFromHtml(html, { markTitle: true }, () => `cms-${counter++}`)
		...
		test('does not mark title when markTitle is false', () => {
			const html = '<html><head><title>My Page Title</title></head><body></body></html>'
			const result = processSeoFromHtml(html, { markTitle: false })
		...
		test('includes source info for title', () => {
			const html = '<html><head>\n<title>My Page Title</title>\n</head><body></body></html>'
			const result = processSeoFromHtml(html, { sourcePath: 'src/pages/index.astro' })
	...
	describe('meta description extraction', () => {
		...
		test('includes source info for description', () => {
	...
	describe('keywords extraction', () => {
		test('extracts and parses meta keywords', () => {
		...
		test('handles keywords with extra spaces', () => {
			const html = '<html><head><meta name="keywords" content="  web ,  development ,  cms  "></head><body></body></html>'
			const result = processSeoFromHtml(html)
	...
	describe('Open Graph extraction', () => {
		...
		test('extracts multiple Open Graph tags', () => {
			const html = `
				<html><head>
		...
		test('does not include openGraph when no OG tags present', () => {
			const html = '<html><head></head><body></body></html>'
			const result = processSeoFromHtml(html)
	...
	describe('Twitter Card extraction', () => {
		...
		test('extracts twitter:description', () => {
		...
		test('handles twitter tags with property attribute', () => {
		...
		test('does not include twitterCard when no Twitter tags present', () => {
			const html = '<html><head></head><body></body></html>'
			const result = processSeoFromHtml(html)
	...
	describe('canonical URL extraction', () => {
		...
		test('handles missing canonical', () => {
			const html = '<html><head></head><body></body></html>'
			const result = processSeoFromHtml(html)
		...
		test('handles canonical without href', () => {
		...
		test('includes source info for canonical', () => {
	...
	describe('JSON-LD extraction', () => {
		...
		test('handles JSON-LD without @type', () => {
			const html = `
				<html><head>
		...
		test('does not parse JSON-LD when parseJsonLd is false', () => {
			const html = `
				<html><head>
					<script type="application/ld+json">{"@type": "Organization"}</script>
				</head><body></body></html>
			`
			const result = processSeoFromHtml(html, { parseJsonLd: false })

			expect(result.seo.jsonLd).toBeUndefined()
		})

		test('handles empty JSON-LD script', () => {
		...
		test('includes source info for JSON-LD', () => {
			const html = `
				<html><head>
		...
		test('extracts JSON-LD from body as well', () => {
			const html = `
				<html><head></head><body>
	...
	describe('complete SEO extraction', () => {
		test('extracts all SEO elements from a complete page', () => {
			const html = `
				<html>
				<head>
					<title>My Page Title</title>
			...
			`
			let counter = 0
			const result = processSeoFromHtml(
				html,
		...
		test('returns empty seo object when no SEO elements present', () => {
